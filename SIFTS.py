...
def get_SIFTS(accession, fields="all"):
    ...
    if callback.status_code != 200:
        raise ValueError(
            f"failed to get {accession} from SIFTS, HTTP status :{callback.status_code}"
        )
...
def getParser():
    parser = argparse.ArgumentParser(
        description="Mappings (as assigned by the SIFTS process) from PDB structures to UniProt, Pfam, InterPro, CATH, SCOP, IntEnz, GO, Ensembl and HMMER accessions (and vice versa)."
    )
    parser.add_argument(
        "-i",
        "--accession",
        ...
        type=str,
        required=True,
        help="PDB id-code OR UniProt accession code OR Pfam accession code OR Interpro accession code OR CATH cathcode OR SCOP sunid OR IntEnz EC code OR GO accession",
    )
